...
with Session(engine) as session:
    ...
    session.execute(text(f'ALTER SEQUENCE {entities.EquipmentEntity.__table__}_id_seq RESTART WITH {len(equipment.models) + 1}'))
    session.commit()

# Add Reservation
with Session(engine) as session:
    from .dev_data import reservation
    to_entity = entities.ReservationEntity.from_model
    session.add_all([to_entity(model) for model in reservation.models])
    session.execute(text(f'ALTER SEQUENCE {entities.ReservationEntity.__table__}_id_seq RESTART WITH {len(reservation.models) + 1}'))
    session.commit()